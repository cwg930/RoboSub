...
def main():
    ...
    movement_client = actionlib.SimpleActionClient('movement_server')
    rospy.loginfo("Created SimpleActionClient.")

    vision_client = actionlib.SimpleActionClient('track_object')
    rospy.loginfo("Created SimpleActionClient.")

	# Declare the top level state
	tasks = smach.StateMachine(outcomes=['success'])
	...
	# Sleeps until shutdown is called
	rospy.spin()
