...
class GateFinder:
    def __init__(self, stereoCameraModel):
        self.stereoCameraModel = stereoCameraModel
    ...
    def normalsFromAllCorners(corners, disparities):
        ...
        for combo in combos: #Find all possible cross products of the available points
            if combo[0] != combo[1] and combo[1] != combo[2] and combo[0] != combo[2]:
                ...
                if np.dot(new, np.array([0,0,-1])) < 0:
                    normals.append(-new)
                else:
                    normals.append(new)
    ...
    def process(self, imageLeftRect, imageRightRect, imageDisparityRect):
        ...
        contours, _ = ThreshAndContour(image, Thresholds(upper=(40,52,120), lower=(20, 30, 80))
        ...
        if len(rects) > 1:
            ...
            if stereoCameraModel is not None: #If we get this, we have(?) stereo camera data, so we can do a full 3d pose
                ...
                roi1Center = (rect1[0][0] + rect1[1][0]*0.4*cos(rect1[3]), rect1[0][0] + rect1[1][1]*0.4*sin(rect1[3]))
                ...
                roi2Center = (rect1[0][0] - rect1[1][0]*0.4*cos(rect1[3]), rect1[0][0] - rect1[1][1]*0.4*sin(rect1[3]))
                ...
                roi3Center = (rect2[0][0] + rect2[1][0]*0.4*cos(rect2[3]), rect2[0][0] + rect2[1][1]*0.4*sin(rect2[3]))
                ...
                roi4Center = (rect2[0][0] - rect2[1][0]*0.4*cos(rect2[3]), rect2[0][0] - rect2[1][1]*0.4*sin(rect2[3]))
                ...
                if disp.count(0) < 1: #we can still get pose with 3 disparity readings
                    corner[1] = self.stereoCameraModel.projectPixelTo3d(roi1Center,disp[1]) #Find all 4 gate corners in 3d
                    corner[2] = self.stereoCameraModel.projectPixelTo3d(roi2Center,disp[2])
                    corner[3] = self.stereoCameraModel.projectPixelTo3d(roi3Center,disp[3])
                    corner[4] = self.stereoCameraModel.projectPixelTo3d(roi4Center,disp[4])
            ...
            if pose is None: #no stereo, center position only
                ...
                projectedRay = self.cameraModel.projectPixelTo3dRay(gateCenter)
                estLocation = tuple(gateDist*x for x in projectedRay)
